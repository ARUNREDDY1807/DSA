...
NODE *remove_song(NODE* head)
{
    ...
    {
        if (cur->min < Min_1->min || (cur->min == Min_1->min && cur->sec < Min_1->sec))
        {
            Min_2 = Min_1;
            Min_1 = cur;
        }
        else if (cur->min < Min_2->min || (cur->min == Min_2->min && cur->sec < Min_2->sec))
        {
            Min_2 = cur;
        }
    }
    ...
    if (Min_1 == Min_2)
    {
        printf("Only one song in the list. Cannot remove two minimum songs.\n");
        return head;
    }
